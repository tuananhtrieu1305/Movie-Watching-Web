...
const isMostlyWhiteImage = (imgElement) => {
  if (!imgElement || imgElement.naturalWidth <= 1 || imgElement.naturalHeight <= 1) {
    return true;
  }
...
const MoviePosterImage = ({ movie }) => {
  const posterCandidates = useMemo(() => {
    if (Array.isArray(movie?.posterCandidates) && movie.posterCandidates.length > 0) {
  ...
  const moveToNextPoster = useCallback(() => {
    setPosterIndex((current) => (current < posterCandidates.length ? current + 1 : current));
  ...
  const handlePosterLoad = useCallback((event) => {
  ...
  if (!activePoster) {
    const fallbackLabel = typeof movie?.title === "string" && movie.title.trim() !== ""
      ? movie.title.trim().charAt(0).toUpperCase()
      : "?";

    return (
      <div className="chatbot-movie-poster-fallback" aria-label="Không có ảnh phim">
...
const SUMMARY_INTENT_REGEX = /(\bt[óo]m\s*t[ắa]t\b|\bt[óo]m\s*l[ượu]c\b|\bn[ộo]i\s*dung\b|\bk[ểe]\s*l[ạa]i\b|\bspoil\b|\bk[ếe]t\s*th[úu]c\b|\bending\b)/i;
...
const normalizeIntentText = (value) => {
  if (typeof value !== "string") return "";
  return normalizeLookupText(value).replace(/[^a-z0-9\s]/g, " ").replace(/\s+/g, " ").trim();
};
...
const isRecommendIntent = (query) => {
  ...
  return RECOMMEND_INTENT_KEYWORDS.some((keyword) => normalizedQuery.includes(keyword));
};
...
const pickCardsByIntent = (cards, query) => {
  ...
  if (queryKey) {
    const directMatch = cards.find(
      (card) => queryKey.includes(card.titleKey) || card.titleKey.includes(queryKey),
    );
  ...
  const scoredCards = cards.map((card) => {
    const score = queryTokens.reduce((acc, token) => (
      card.titleKey.includes(token) ? acc + 1 : acc
    ), 0);
...
const buildMovieCardFromProduction = (production) => {
  ...
  const title = typeof production?.title === "string" && production.title.trim() !== ""
  ...
  const country = typeof production?.country === "string" ? production.country.trim() : "";
  const language = typeof production?.language === "string" ? production.language.trim() : "";
  const description = typeof production?.description === "string" ? production.description.trim() : "";
...
const ChatbotWidget = () => {
  ...
  const hydrateMovieCardsFromSourceIds = useCallback(async (sourceIds) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const normalizedAnswerTextRaw = stripLegacyLinkLines(answerTextRaw) || answerTextRaw;
      ...
      const shouldUseRecommendationLead =
        movieCards.length > 0 && (
          ...
          (movieCards.length >= 2 && isRecommendationAnswerText(normalizedAnswerTextRaw))
        );

      const answerText =
        shouldUseRecommendationLead
                          ...
                          <>
                            ...
                            <div className="chatbot-movie-info">
                              ...
                              {Array.isArray(movie.tags) && movie.tags.length > 0 && (
                                <div className="chatbot-movie-tags">
                                  {movie.tags.map((tag, tagIndex) => (
                                    <span key={`${movie.id}-tag-${tagIndex}`} className="chatbot-movie-tag">{tag}</span>
                                  ))}
                                </div>
                              )}
                              {movie.description && (
                                <div className="chatbot-movie-description">{movie.description}</div>
                              )}
                            </div>
                          </>
                        );
