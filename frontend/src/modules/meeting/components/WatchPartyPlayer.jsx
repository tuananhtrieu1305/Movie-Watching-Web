...
export default function WatchPartyPlayer({
  ...
  useEffect(() => {
    ...
    socket.on("party_state", (state) => {
      ...
      if (state.currentMovie) {
        ...
        if (!isHost) {
          console.log(
            `[WatchParty] Guest bù ping: ${pingMs}ms, nhảy tới ${compensatedTime}`,
          );
    ...
    socket.on("guest_sync_video", (data) => {
      ...
      // Bù ping để tính vị trí thực tế của Host
      ...
      if (type === "seek" || type === "play" || type === "heartbeat") {
        const drift = vt.currentTime - targetTime; // dương = guest nhanh hơn, âm = guest chậm hơn
        const absDrift = Math.abs(drift);

        if (absDrift > 5) {
          // Lệch quá lớn → buộc phải Hard Seek (sẽ buffer nhưng không còn cách nào khác)
          console.log(
            `[Sync] Hard Seek: lệch ${drift.toFixed(2)}s → nhảy tới ${targetTime.toFixed(2)}s`,
          );
          vt.currentTime = targetTime;
          vt.playbackRate = 1.0;
        } else if (absDrift > 0.3) {
          // Lệch vừa: điều chỉnh tốc độ phát → không cần seek, không có buffer!
          if (drift < 0) {
            // Guest chậm hơn Host → tăng tốc độ
            vt.playbackRate = 2;
            console.log(
              `[Sync] Guest chậm ${absDrift.toFixed(2)}s → tăng lên 1.1x`,
            );
          } else {
            // Guest nhanh hơn Host → giảm tốc độ
            vt.playbackRate = 0.9;
            console.log(
              `[Sync] Guest nhanh ${absDrift.toFixed(2)}s → giảm xuống 0.9x`,
            );
          }
        } else {
          // Nằm trong ngưỡng OK → khôi phục tốc độ bình thường
          vt.playbackRate = 1.0;
        }
      ...
      if (type === "play") {
        ...
        vt.pause();
        vt.playbackRate = 1.0; // Reset tốc độ khi pause
      }
    ...
    socket.on("party_ended", (data) => {
      ...
      // Xóa session để tránh tự động vào lại phòng đã đóng
  ...
  // 4. Heartbeat Sync (Host gửi mỗi 2s để phát hiện lệch sớm)
  useEffect(() => {
    ...
    }, 2000); // Tăng tần suất lên 2s để Speed Adjustment phát hiện lệch nhanh hơn
  ...
  const handleGuestJoinTheater = () => {
    console.log(
      "[WatchParty] Guest bắt đầu vào rạp, nhảy tới:",
      partyState.currentTime,
    );
  ...
  return (
    <div className="h-full w-full flex flex-col relative bg-[#121212]">
      ...
      <div className="absolute top-4 right-4 z-50 flex items-center gap-3">
        ...
        <div className="flex items-center bg-gray-900/80 border border-gray-700 rounded-full px-4 py-1.5 backdrop-blur-md shadow-2xl group transition-all hover:border-[#ffdd95]">
          ...
          <Tooltip title="Copy mã phòng">
            <Button
              ...
              size="small"
              icon={
                <CopyOutlined className="text-gray-400 group-hover:text-[#ffdd95]" />
              }
        ...
        <Tooltip
          title={
            isTheaterMode ? "Thu nhỏ phòng chiếu" : "Phóng to toàn màn hình"
          }
        >
          <Button
            type="primary"
          ...
          >
            ...
            {guestWaitingAutoplay && !isHost && (
              <div className="absolute inset-0 z-40 bg-black/80 flex flex-col items-center justify-center backdrop-blur-sm">
                ...
                <Button
                  type="primary"
                  size="large"
                  className="animate-bounce !bg-[#ffdd95] !text-[#111] border-none !font-bold"
            ...
            {isBuffering && (
              <div className="absolute inset-0 z-30 bg-black/40 flex flex-col items-center justify-center backdrop-blur-[2px] transition-all">
                ...
                <div className="mt-4 text-[#ffdd95] font-medium animate-pulse">
                  Đang đồng bộ phim...
                </div>
              </div>
            )}
            ...
            {/* Custom Controls cho Guest (Volume + Fullscreen) */}
          ...
          {isHost ? (
            <>
              <div className="text-center mb-10">
                ...
                <p className="text-gray-400 mt-2 text-lg">
                  Bạn là Chủ Phòng. Hãy chọn một bộ phim dưới đây để bắt đầu
                  phát cho mọi người.
                </p>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                {movieList.map((movie) => (
                  ...
                  >
                    <div className="truncate text-white font-semibold text-lg">
                      {movie.title}
                    </div>
            ...
            <div className="h-full flex flex-col items-center justify-center animate-pulse">
              ...
              <p className="text-gray-400 text-lg">
                Bạn đã vào phòng thành công. Phim sẽ bắt đầu ngay khi chủ phòng
                lựa chọn.
              </p>
            </div>
          )}
