      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((plan, index) => (
            <div
              key={index}
              className={`relative bg-[#2a2a2d] rounded-2xl p-8 shadow-lg border transition-transform hover:scale-105 ${
                plan.highlight
            ...
            >
              ...
              {/* Button */}
              <button
                className={`w-full py-3 px-6 rounded-lg font-bold mt-auto cursor-pointer transition-all ${plan.buttonStyle}`}
                disabled={plan.isCurrent}
                onClick={() =>
                  !plan.isCurrent &&
                  navigate(`/user/checkout?plan=${plan.name.toLowerCase()}`)
                }
              >
                {plan.buttonText}
              </button>
        ...
        <p className="text-center text-gray-500 text-sm mt-10">
          Thanh toán an toàn qua MoMo, ZaloPay, Visa/Mastercard. Hủy bất cứ lúc
          nào.
        </p>
      </div>
    </div>
