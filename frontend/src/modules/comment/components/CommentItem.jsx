...
const CommentItem = ({ data, productionId, accessToken, currentUserId, depth = 0 }) => {
  ...
  useEffect(() => {
    const handleReactionUpdated = (payload) => {
      if (String(payload.commentId) !== String(data.id)) return;
      setLikesCount(payload.likesCount || 0);
      setDislikesCount(payload.dislikesCount || 0);
    };

    const handleCommentDeleted = (payload) => {
      if (String(payload.commentId) === String(data.id)) {
        setDeleted(true);
        return;
      }

      if (String(payload.parentId) === String(data.id)) {
        setReplyCount((count) => Math.max(count - 1, 0));
        setReplies((prev) =>
          prev.filter((reply) => String(reply.id) !== String(payload.commentId)),
        );
      }
    };
