...
const CommentSection = ({ productionId }) => {
  ...
  useEffect(() => {
    ...
    const joinProductionRoom = () => {
      socket.emit("join_production", productionId);
    };

    const handleNewComment = (comment) => {
      if (String(comment.production_id) !== String(productionId)) return;
      setComments((prev) => {
    ...
    const handleCommentDeleted = (payload) => {
      if (String(payload.productionId) !== String(productionId)) return;
      if (payload.parentId) return;
      setComments((prev) =>
        prev.filter((comment) => String(comment.id) !== String(payload.commentId)),
      );
    };

    socket.on("connect", joinProductionRoom);
    ...
    return () => {
      ...
      socket.off("connect", joinProductionRoom);
